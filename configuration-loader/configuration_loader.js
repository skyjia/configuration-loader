...
function createLoader(defaultConfigFilePath, currentConfigFilePath){
...
ConfigurationLoader.prototype.reload = function () {
    ...
    if (!fs.existsSync(this.defaultConfigFilePath)) {
        throw new Error("Default configuration file is missing.");
    }

    if (!fs.existsSync(this.currentConfigFilePath)) {
        throw new Error("Current configuration file is missing.");
    }
